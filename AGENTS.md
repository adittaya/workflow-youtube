...
- **Destination redirect wrapper detection**: when popup opens a LinkedIn/Google/Facebook/Twitter redirect URL, waits up to 20s for it to resolve to the actual final URL before capturing.
...
- **Ad overlay close**: `#block-cont-1 > div:nth-child(1)` "X" button must be clicked on every article page to dismiss ad overlays before interacting with template buttons.
- **wistfulseverely.com tracking wrapper**: get-link popup opens wistfulseverely.com first (tracking), then redirects to actual destination. Added to DEST_PATTERNS and redirect wrapper detection.
