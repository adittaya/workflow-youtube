...
# _config_json_get — portable JSON value extraction (POSIX-compatible, no grep -P)
_config_json_get() {
    local key="$1"
    local line result

    # Find the line containing our key (POSIX grep, no -P)
    line="$(echo "$_CONFIG_DATA" | grep "\"${key}\"[[:space:]]*:" 2>/dev/null | head -1)"
    if [[ -z "$line" ]]; then
        return 1
    fi
...
_config_json_set() {
    ...
    if echo "$_CONFIG_DATA" | grep -qE "$pattern" 2>/dev/null; then
    ...
    echo "$_CONFIG_DATA" | awk -v k="$key" -v v="$escaped_value" '{
...
config_has() {
    ...
    local pattern="\"${key}\"[[:space:]]*:"
    echo "$_CONFIG_DATA" | grep -qE "$pattern" 2>/dev/null
}
...
# _config_json_remove — portable JSON key removal (POSIX-compatible, no GNU sed)
_config_json_remove() {
    local key="$1"
    # Remove the key-value line and clean up trailing commas (awk for portability)
    echo "$_CONFIG_DATA" | awk -v k="$key" '
    $0 ~ "\"" k "\"[[:space:]]*:" { next }
    { lines[NR] = $0; last = NR }
    END {
